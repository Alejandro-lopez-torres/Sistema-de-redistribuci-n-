    ...
    try:
        ...
        for micro_red in df['micro red'].unique():
            ...
            for index, row in df_micro_red.iterrows():
                ...
                if stock_actual > 0:
                    for mes in meses:
                        if mes in df.columns:
                            ...
                            stock_a_dar += salidas
                            stock_a_dar = min(stock_a_dar, stock_actual)
                ...
                if stock_a_dar > 0 and row['establecimiento'] in otros_establecimientos['establecimiento'].values:
                    stock_reutilizado = min(stock_a_dar, stock_actual)
                    stock_a_dar -= stock_reutilizado
                    ...
                    for _, otro_row in otros_establecimientos.iterrows():
                        ...
                        if stock_a_dar > 0:
                            for mes in meses:
                                if mes in df.columns:
                                    salidas = pd.to_numeric(row[mes], errors='coerce')
                                    cantidad_a_recibir = min(salidas, stock_disponible, stock_a_dar)

                                    stock_a_recibir += cantidad_a_recibir
                                    stock_a_dar -= cantidad_a_recibir
                ...
                redistribucion.append({
                    ...
                    'STOCK A RECIBIR': stock_a_recibir if stock_a_recibir > 0 else "",
                    'STOCK REUTILIZADO': stock_reutilizado if stock_reutilizado > 0 else "",
                    'STOCK TOTAL': stock_total,
                    'STOCK FINAL': stock_final,
                    'TOTAL': total,
                    'ESTABLECIMIENTO DE DONDE SE EXTRAE EL STOCK': establecimiento_origen,
                    'ESTABLECIMIENTO A DONDE SE TRASPASA EL STOCK': establecimiento_destino if stock_a_dar > 0 else "NO SE TRASPASAN STOCK",
...
class App(QMainWindow):
    def __init__(self):
        ...
        self.entry_buscar_micro_red = QLineEdit()
        self.layout.addWidget(self.entry_buscar_micro_red, 1, 1)

        self.boton_buscar_micro_red = QPushButton("Buscar")
        ...
        self.layout.addWidget(self.label_buscar_medicamento, 2, 0)
        self.entry_buscar_medicamento = QLineEdit()
        self.layout.addWidget(self.entry_buscar_medicamento, 2, 1)
        ...
        self.layout.addWidget(self.boton_buscar_medicamento, 2, 2)

        self.label_buscar_disponibilidad = QLabel("Buscar Disponibilidad:")
        self.layout.addWidget(self.label_buscar_disponibilidad, 3, 0)
        ...
        self.layout.addWidget(self.boton_buscar_disponibilidad, 5, 0, 1, 3)

        self.label_info = QLabel("")
        self.layout.addWidget(self.label_info, 6, 0, 1, 3)

        self.table_widget = QTableWidget()
        self.layout.addWidget(self.table_widget, 7, 0, 1, 3)

        self.progress_bar = QProgressBar(self)
        self.layout.addWidget(self.progress_bar, 8, 0, 1, 3)
    ...
    def filtrar_micro_red(self):
        self.filtrar_tabla('MICRO RED', self.entry_buscar_micro_red.text().upper())
